configuration for "PIC18F2480" is
end configuration;
--
testbench for "PIC18F2480" is
begin
  test_timeout: process is
    begin
      wait for 163 ms;
      report("flim_nv_write_test: TIMEOUT");
      report(PC); -- Crashes simulator, MDB will report current source line
      PC <= 0;
      wait;
    end process test_timeout;
    --
  flim_nv_write_test: process is
    type test_result is (pass, fail);
    variable test_state  : test_result;
    file     data_file   : text;
    variable file_stat   : file_open_status;
    variable file_line   : string;
    begin
      report("flim_nv_write_test: START");
      test_state := pass;
      RA2 <= '1'; -- Setup button not pressed
      RA1 <= '1'; -- Learn off
      RA0 <= '1'; -- Unlearn off
      --
      wait until RB6 == '1'; -- Booted into FLiM
      report("flim_nv_write_test: Yellow LED (FLiM) on");
      --
      file_open(file_stat, data_file, "./data/flim_ignore.dat", read_mode);
      if file_stat != open_ok then
        report("flim_nv_write_test: Failed to open ignored addresses data file");
        report("flim_nv_write_test: FAIL");
        PC <= 0;
        wait;
      end if;
      --
      report("flim_nv_write_test: Ignore requests not addressed to node");
      while endfile(data_file) == false loop
        readline(data_file, file_line);
        report(file_line);
        RXB0D0 <= 16#96#;           -- NVSET, CBUS set node variable by index
        read(data_file, RXB0D1, 1); -- NN high
        read(data_file, RXB0D2, 1); -- NN low
        RXB0D3 <= 1;                -- Index
        RXB0D4 <= 1;                -- Value
        RXB0CON.RXFUL <= '1';
        RXB0DLC.DLC3 <= '1';
        COMSTAT <= 16#80#;
        CANSTAT <= 16#0C#;
        PIR3.RXB0IF <= '1';
        --
        TXB1CON.TXREQ <= '0';
        --
        wait until RXB0CON.RXFUL == '0';
        COMSTAT <= 0;
        --
        wait until TXB1CON.TXREQ == '1' for 2 ms; -- Test if response sent
        if TXB1CON.TXREQ == '1' then
          report("flim_nv_write_test: Unexpected response");
          test_state := fail;
        end if;
      end loop;
      --
      file_close(data_file);
      --
      report("flim_nv_write_test: Check original output 2 pulse duration");
      RXB0D0 <= 16#71#;      -- NVRD, CBUS read node variable by index
      RXB0D1 <= 4;           -- NN high
      RXB0D2 <= 2;           -- NN low
      RXB0D3 <= 7;           -- Output 2 pulse duration
      RXB0CON.RXFUL <= '1';
      RXB0DLC.DLC3 <= '1';
      COMSTAT <= 16#80#;
      CANSTAT <= 16#0C#;
      PIR3.RXB0IF <= '1';
      --
      TXB1CON.TXREQ <= '0';
      --
      wait until RXB0CON.RXFUL == '0';
      COMSTAT <= 0;
      --
      wait until TXB1CON.TXREQ == '1';
      if TXB1D0 != 16#97# then -- NVANS, CBUS node variable response
        report("flim_nv_write_test: Sent wrong response");
        test_state := fail;
      end if;
      if TXB1D1 != 4 then
        report("flim_nv_write_test: Sent wrong Node Number (high)");
        test_state := fail;
      end if;
      if TXB1D2 != 2 then
        report("flim_nv_write_test: Sent wrong Node Number (low)");
        test_state := fail;
      end if;
      if TXB1D3 != 7 then
        report("flim_nv_write_test: Wrong node variable index");
        test_state := fail;
      end if;
      if TXB1D4 != 0 then
        report("flim_nv_write_test: Unexpected pulse duration value");
        test_state := fail;
      end if;
      --
      report("flim_nv_write_test: Change output 2 pulse duration");
      RXB0D0 <= 16#96#;      -- NVSET, CBUS set node variable by index
      RXB0D1 <= 4;           -- NN high
      RXB0D2 <= 2;           -- NN low
      RXB0D3 <= 7;           -- Output 2 pulse duration
      RXB0D4 <= 1;           -- 10 milliseconds
      RXB0CON.RXFUL <= '1';
      RXB0DLC.DLC3 <= '1';
      COMSTAT <= 16#80#;
      CANSTAT <= 16#0C#;
      PIR3.RXB0IF <= '1';
      --
      TXB1CON.TXREQ <= '0';
      --
      wait until RXB0CON.RXFUL == '0';
      COMSTAT <= 0;
      --
      wait until TXB1CON.TXREQ == '1';
      if TXB1D0 != 16#59# then -- WRACK, CBUS write acknowledge response
        report("flim_nv_write_test: Sent wrong response");
        test_state := fail;
      end if;
      if TXB1D1 != 4 then
        report("flim_nv_write_test: Sent wrong Node Number (high)");
        test_state := fail;
      end if;
      if TXB1D2 != 2 then
        report("flim_nv_write_test: Sent wrong Node Number (low)");
        test_state := fail;
      end if;
      --
      report("flim_nv_write_test: Read back new output 2 pulse duration");
      RXB0D0 <= 16#71#;      -- NVRD, CBUS read node variable by index
      RXB0D1 <= 4;           -- NN high
      RXB0D2 <= 2;           -- NN low
      RXB0D3 <= 7;           -- Output 2 pulse duration
      RXB0CON.RXFUL <= '1';
      RXB0DLC.DLC3 <= '1';
      COMSTAT <= 16#80#;
      CANSTAT <= 16#0C#;
      PIR3.RXB0IF <= '1';
      --
      TXB1CON.TXREQ <= '0';
      --
      wait until RXB0CON.RXFUL == '0';
      COMSTAT <= 0;
      --
      wait until TXB1CON.TXREQ == '1';
      if TXB1D0 != 16#97# then -- NVANS, CBUS node variable response
        report("flim_nv_write_test: Sent wrong response");
        test_state := fail;
      end if;
      if TXB1D1 != 4 then
        report("flim_nv_write_test: Sent wrong Node Number (high)");
        test_state := fail;
      end if;
      if TXB1D2 != 2 then
        report("flim_nv_write_test: Sent wrong Node Number (low)");
        test_state := fail;
      end if;
      if TXB1D3 != 7 then
        report("flim_nv_write_test: Wrong node variable index");
        test_state := fail;
      end if;
      if TXB1D4 != 1 then
        report("flim_nv_write_test: Incorrect pulse duration value");
        test_state := fail;
      end if;
      --
      report("flim_nv_write_test: Short on 0x0201,0x0204, trigger output 2");
      RXB0D0 <= 16#98#;      -- ASON, CBUS short on
      RXB0D1 <= 2;           -- NN high
      RXB0D2 <= 1;           -- NN low
      RXB0D3 <= 2;           -- Event number high
      RXB0D4 <= 4;           -- Event number low
      RXB0CON.RXFUL <= '1';
      RXB0DLC.DLC3 <= '1';
      COMSTAT <= 16#80#;
      CANSTAT <= 16#0C#;
      PIR3.RXB0IF <= '1';
      --
      wait until RXB0CON.RXFUL == '0';
      COMSTAT <= 0;
      --
      wait until PORTC != 0;
      if PORTC == 64 then
        report("flim_nv_write_test: Triggered output 2");
      else
        report("flim_nv_write_test: Wrong output");
        test_state := fail;
      end if;
      wait until PORTC == 0 for 141 ms;
      if PORTC == 64 then
        report("flim_nv_write_test: Output 2 not pulsed");
        test_state := fail;
      end if;
      --
      report("flim_nv_write_test: Node variable index too low");
      RXB0D0 <= 16#96#;      -- NVSET, CBUS set node variable by index
      RXB0D1 <= 4;           -- NN high
      RXB0D2 <= 2;           -- NN low
      RXB0D3 <= 0;
      RXB0D4 <= 0;
      RXB0CON.RXFUL <= '1';
      RXB0DLC.DLC3 <= '1';
      COMSTAT <= 16#80#;
      CANSTAT <= 16#0C#;
      PIR3.RXB0IF <= '1';
      --
      wait until RXB0CON.RXFUL == '0';
      COMSTAT <= 0;
      --
      -- FIXME No CMDERR
      --TXB1CON.TXREQ <= '0';
      --wait until TXB1CON.TXREQ == '1';
      --if TXB1D0 != 16#6F# then -- CMDERR, CBUS error response
      --  report("flim_param_test: Sent wrong response");
      --  test_state := fail;
      --end if;
      --if TXB1D1 != 4 then
      --  report("flim_param_test: Sent wrong Node Number (high)");
      --  test_state := fail;
      --end if;
      --if TXB1D2 != 2 then
      --  report("flim_param_test: Sent wrong Node Number (low)");
      --  test_state := fail;
      --end if;
      --if TXB1D3 != 10 then -- Invalid node variable index
      --  report("flim_param_test: Sent wrong error number");
      --  test_state := fail;
      --end if;
      --
      report("flim_nv_write_test: Node variable index too high");
      RXB0D0 <= 16#96#;      -- NVSET, CBUS set node variable by index
      RXB0D1 <= 4;           -- NN high
      RXB0D2 <= 2;           -- NN low
      RXB0D3 <= 13;
      RXB0D4 <= 0;
      RXB0CON.RXFUL <= '1';
      RXB0DLC.DLC3 <= '1';
      COMSTAT <= 16#80#;
      CANSTAT <= 16#0C#;
      PIR3.RXB0IF <= '1';
      --
      wait until RXB0CON.RXFUL == '0';
      COMSTAT <= 0;
      --
      -- FIXME No CMDERR
      --TXB1CON.TXREQ <= '0';
      --wait until TXB1CON.TXREQ == '1';
      --if TXB1D0 != 16#6F# then -- CMDERR, CBUS error response
      --  report("flim_param_test: Sent wrong response");
      --  test_state := fail;
      --end if;
      --if TXB1D1 != 4 then
      --  report("flim_param_test: Sent wrong Node Number (high)");
      --  test_state := fail;
      --end if;
      --if TXB1D2 != 2 then
      --  report("flim_param_test: Sent wrong Node Number (low)");
      --  test_state := fail;
      --end if;
      --if TXB1D3 != 10 then -- Invalid node variable index
      --  report("flim_param_test: Sent wrong error number");
      --  test_state := fail;
      --end if;
      --
     if test_state == pass then
        report("flim_nv_write_test: PASS");
      else
        report("flim_nv_write_test: FAIL");
      end if;          
      PC <= 0;
      wait;
    end process flim_nv_write_test;
end testbench;
